...
class PolygonModel:
    ...
    def draw_poly(self, fill = False):
        pixel_points = [to_pixels(x,y) for x,y in self.transformed()]
        # pygame.draw.aalines(self.screen, self.color, True, pixel_points, 10)
        if fill:
            pygame.draw.polygon(self.screen, self.color, pixel_points, 0)
        else:
            pygame.draw.lines(self.screen, self.color, True, pixel_points, 2)
    ...
    def collide_with(self, other_polygon) -> bool:
        for other_seg in other_polygon.segments():
            if self.intersect_with(other_seg):
                return True
        return False
    def gravitational_field(self, source):
        relative_position = (self.x - source.x, self.y - source.y)
        return vectors.scale(- source.gravity, relative_position)

    def move(self, milliseconds, thrust_vector=(0,0), gravity_source = None):
        ax, ay = thrust_vector
        if gravity_source:
            gx, gy = self.gravitational_field(gravity_source)
        else:
            gx, gy = (0, 0)
        ax += gx
        ay += gy
...
class BlackHole(PolygonModel):
    def __init__(self, screen, gravity) -> None:
        points = [vectors.to_cartesian((0.5, 2 * math.pi * i / 20)) for i in range(0,20)]
        super().__init__(screen, "black", points)
...
def run():
    ...
    acceleration = 3
    blackhole = BlackHole(screen, 0.1)
    while not done:
        screen.fill("white")
        blackhole.draw()
        clock.tick()
        ...
        ship.move(milliseconds, gravity_source=blackhole)
        ship.draw()
        for asteroid in asteroids:
            asteroid.move(milliseconds, gravity_source=blackhole)
            asteroid.draw()
